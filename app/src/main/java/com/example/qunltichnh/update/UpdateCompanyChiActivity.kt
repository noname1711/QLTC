...
class UpdateCompanyChiActivity : AppCompatActivity() {

    private lateinit var networkChangeReceiver: NetworkChangeReceiver

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_update_company_chi)

        setValueToView()  //hàm set giá trị để hiển thị
        val btnDeleteChi = findViewById<Button>(R.id.btnDeleteChiCompany)
        btnDeleteChi.setOnClickListener{
            deleteData(intent.getStringExtra("idChi").toString())   //hàm xóa data theo idChi
        }
        val btnUpdateChi = findViewById<Button>(R.id.btnUpdateChiCompany)
        ...
        if (!initialNetworkStatus) {
            Toast.makeText(this@UpdateCompanyChiActivity, R.string.loss_internet, Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun openUpdateDialogChi(idChi: String, khoanChi: String) {
        ...
        btnUpdateDataChi.setOnClickListener {
            ...
            val tvDateChi = findViewById<TextView>(R.id.tvDateChiCompany)
            val tvTimeChi = findViewById<TextView>(R.id.tvTimeChiCompany)
            val tvKhoanChi = findViewById<TextView>(R.id.tvKhoanChiCompany)
            val tvLuongChi = findViewById<TextView>(R.id.tvLuongChiCompany)
    ...
    private fun updateDataChi(idChi: String, dateChi: String, timeChi: String, khoanChi: String, luongChi: String) {
        ...
        val employeeDataChi = dbRef.child(userId).child("Company").child("Chi")
        ...
        employeeDataChi.child(idChi).setValue(empInfoChi).addOnSuccessListener{
            Toast.makeText(this@UpdateCompanyChiActivity,"Đã chỉnh sửa thông tin", Toast.LENGTH_SHORT).show()
        }.addOnFailureListener { err ->
            Toast.makeText(this@UpdateCompanyChiActivity,"Xóa lỗi ${err.message}", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun deleteData(idChi: String) {
        ...
        employeeDataChi.child(idChi).removeValue().addOnSuccessListener {
            Toast.makeText(this@UpdateCompanyChiActivity,"Đã xóa khoản chi này", Toast.LENGTH_SHORT).show()
