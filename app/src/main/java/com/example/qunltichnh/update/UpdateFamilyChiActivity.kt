...
class UpdateFamilyChiActivity : AppCompatActivity() {

    private lateinit var networkChangeReceiver: NetworkChangeReceiver

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_update_family_chi)

        setValueToView()  //hàm set giá trị để hiển thị
        val btnDeleteChi = findViewById<Button>(R.id.btnDeleteChiFamily)
        btnDeleteChi.setOnClickListener{
            deleteData(intent.getStringExtra("idChi").toString())   //hàm xóa data theo idChi
        }
        val btnUpdateChi = findViewById<Button>(R.id.btnUpdateChiFamily)
        ...
        if (!initialNetworkStatus) {
            Toast.makeText(this@UpdateFamilyChiActivity, R.string.loss_internet, Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun openUpdateDialogChi(idChi: String, khoanChi: String) {
        ...
        btnUpdateDataChi.setOnClickListener {
            ...
            val tvDateChi = findViewById<TextView>(R.id.tvDateChiFamily)
            val tvTimeChi = findViewById<TextView>(R.id.tvTimeChiFamily)
            val tvKhoanChi = findViewById<TextView>(R.id.tvKhoanChiFamily)
            val tvLuongChi = findViewById<TextView>(R.id.tvLuongChiFamily)
    ...
    private fun updateDataChi(idChi: String, dateChi: String, timeChi: String, khoanChi: String, luongChi: String) {
        ...
        val employeeDataChi = dbRef.child(userId).child("Family").child("Chi")
        ...
        employeeDataChi.child(idChi).setValue(empInfoChi).addOnSuccessListener{
            Toast.makeText(this@UpdateFamilyChiActivity,"Đã chỉnh sửa thông tin", Toast.LENGTH_SHORT).show()
        }.addOnFailureListener { err ->
            Toast.makeText(this@UpdateFamilyChiActivity,"Xóa lỗi ${err.message}", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun deleteData(idChi: String) {
        ...
        employeeDataChi.child(idChi).removeValue().addOnSuccessListener {
            Toast.makeText(this@UpdateFamilyChiActivity,"Đã xóa khoản chi này", Toast.LENGTH_SHORT).show()
