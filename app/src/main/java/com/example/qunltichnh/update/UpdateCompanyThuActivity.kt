...
class UpdateCompanyThuActivity : AppCompatActivity() {

    private lateinit var networkChangeReceiver: NetworkChangeReceiver

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_update_company_thu)

        setValueToView()  //hàm set giá trị để hiển thị
        val btnDeleteThu = findViewById<Button>(R.id.btnDeleteThuCompany)
        btnDeleteThu.setOnClickListener{
            deleteData(intent.getStringExtra("idThu").toString())   //hàm xóa data theo idThu
        }
        val btnUpdateThu = findViewById<Button>(R.id.btnUpdateThuCompany)
        ...
        if (!initialNetworkStatus) {
            Toast.makeText(this@UpdateCompanyThuActivity, R.string.loss_internet, Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun openUpdateDialogThu(idThu: String, khoanThu: String) {
        ...
        btnUpdateDataThu.setOnClickListener {
            ...
            val tvDateThu = findViewById<TextView>(R.id.tvDateThuCompany)
            val tvTimeThu = findViewById<TextView>(R.id.tvTimeThuCompany)
            val tvKhoanThu = findViewById<TextView>(R.id.tvKhoanThuCompany)
            val tvLuongThu = findViewById<TextView>(R.id.tvLuongThuCompany)
    ...
    private fun updateDataThu(idThu: String, dateThu: String, timeThu: String, khoanThu: String, luongThu: String) {
        ...
        val employeeDataThu = dbRef.child(userId).child("Company").child("Thu")
        ...
        employeeDataThu.child(idThu).setValue(empInfoThu).addOnSuccessListener{
            Toast.makeText(this@UpdateCompanyThuActivity,"Đã chỉnh sửa thông tin", Toast.LENGTH_SHORT).show()
        }.addOnFailureListener { err ->
            Toast.makeText(this@UpdateCompanyThuActivity,"Xóa lỗi ${err.message}", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun deleteData(idThu : String) {
        ...
        employeeDataThu.child(idThu).removeValue().addOnSuccessListener {
            Toast.makeText(this@UpdateCompanyThuActivity,"Đã xóa khoản thu này", Toast.LENGTH_SHORT).show()
