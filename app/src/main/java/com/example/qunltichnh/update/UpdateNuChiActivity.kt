...
class UpdateNuChiActivity : AppCompatActivity() {

    private lateinit var networkChangeReceiver: NetworkChangeReceiver

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_update_nu_chi)

        setValueToView()  //hàm set giá trị để hiển thị
        val btnDeleteChi = findViewById<Button>(R.id.btnDeleteChiNu)
        btnDeleteChi.setOnClickListener{
            deleteData(intent.getStringExtra("idChi").toString())   //hàm xóa data theo idChi
        }
        val btnUpdateChi = findViewById<Button>(R.id.btnUpdateChiNu)
        ...
        if (!initialNetworkStatus) {
            Toast.makeText(this@UpdateNuChiActivity, R.string.loss_internet, Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun openUpdateDialogChi(idChi: String, khoanChi: String) {
        ...
        btnUpdateDataChi.setOnClickListener {
            ...
            val tvDateChi = findViewById<TextView>(R.id.tvDateChiNu)
            val tvTimeChi = findViewById<TextView>(R.id.tvTimeChiNu)
            val tvKhoanChi = findViewById<TextView>(R.id.tvKhoanChiNu)
            val tvLuongChi = findViewById<TextView>(R.id.tvLuongChiNu)
    ...
    private fun updateDataChi(idChi: String, dateChi: String, timeChi: String, khoanChi: String, luongChi: String) {
        ...
        val employeeDataChi = dbRef.child(userId).child("Nu").child("Chi")
        ...
        employeeDataChi.child(idChi).setValue(empInfoChi).addOnSuccessListener{
            Toast.makeText(this@UpdateNuChiActivity,"Đã chỉnh sửa thông tin", Toast.LENGTH_SHORT).show()
        }.addOnFailureListener { err ->
            Toast.makeText(this@UpdateNuChiActivity,"Xóa lỗi ${err.message}", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun deleteData(idChi: String) {
        ...
        employeeDataChi.child(idChi).removeValue().addOnSuccessListener {
            Toast.makeText(this@UpdateNuChiActivity,"Đã xóa khoản chi này", Toast.LENGTH_SHORT).show()
