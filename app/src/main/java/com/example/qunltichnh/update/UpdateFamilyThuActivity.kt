...
class UpdateFamilyThuActivity : AppCompatActivity() {

    private lateinit var networkChangeReceiver: NetworkChangeReceiver

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_update_family_thu)

        setValueToView()  //hàm set giá trị để hiển thị
        val btnDeleteThu = findViewById<Button>(R.id.btnDeleteThuFamily)
        btnDeleteThu.setOnClickListener{
            deleteData(intent.getStringExtra("idThu").toString())   //hàm xóa data theo idThu
        }
        val btnUpdateThu = findViewById<Button>(R.id.btnUpdateThuFamily)
        ...
        if (!initialNetworkStatus) {
            Toast.makeText(this@UpdateFamilyThuActivity, R.string.loss_internet, Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun openUpdateDialogThu(idThu: String, khoanThu: String) {
        ...
        btnUpdateDataThu.setOnClickListener {
            ...
            val tvDateThu = findViewById<TextView>(R.id.tvDateThuFamily)
            val tvTimeThu = findViewById<TextView>(R.id.tvTimeThuFamily)
            val tvKhoanThu = findViewById<TextView>(R.id.tvKhoanThuFamily)
            val tvLuongThu = findViewById<TextView>(R.id.tvLuongThuFamily)
    ...
    private fun updateDataThu(idThu: String, dateThu: String, timeThu: String, khoanThu: String, luongThu: String) {
        ...
        val employeeDataThu = dbRef.child(userId).child("Family").child("Thu")
        ...
        employeeDataThu.child(idThu).setValue(empInfoThu).addOnSuccessListener{
            Toast.makeText(this@UpdateFamilyThuActivity,"Đã chỉnh sửa thông tin", Toast.LENGTH_SHORT).show()
        }.addOnFailureListener { err ->
            Toast.makeText(this@UpdateFamilyThuActivity,"Xóa lỗi ${err.message}", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun deleteData(idThu : String) {
        ...
        employeeDataThu.child(idThu).removeValue().addOnSuccessListener {
            Toast.makeText(this@UpdateFamilyThuActivity,"Đã xóa khoản thu này", Toast.LENGTH_SHORT).show()
