...
class DoThiCompanyActivity : AppCompatActivity() {

    private lateinit var combinedChartCompany: CombinedChart
    private lateinit var dbRef: DatabaseReference
    private lateinit var auth: FirebaseAuth
    private lateinit var edtChonNamCompany: EditText
    private lateinit var btnTruyVanCompany: Button

    private lateinit var networkChangeReceiver: NetworkChangeReceiver

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_do_thi_company)

        combinedChartCompany = findViewById(R.id.combinedChartCompany)
        edtChonNamCompany = findViewById(R.id.edtChonNamCompany)
        btnTruyVanCompany = findViewById(R.id.btnTruyVanCompany)
        ...
        btnTruyVanCompany.setOnClickListener {
            val year = edtChonNamCompany.text.toString().trim()
            if (year.isNotEmpty()) {
                fetchChartData(year)
            } else {
                Toast.makeText(this@DoThiCompanyActivity, "Vui lòng chọn năm", Toast.LENGTH_SHORT).show()
            }
        }
        ...
        if (!initialNetworkStatus) {
            Toast.makeText(this@DoThiCompanyActivity, R.string.loss_internet, Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun fetchChartData(year: String) {
        ...
        currentUser?.let { user ->
            ...
            val thuRef = dbRef.child(userId!!).child("Company").child("Thu")
            val chiRef = dbRef.child(userId).child("Company").child("Chi")
