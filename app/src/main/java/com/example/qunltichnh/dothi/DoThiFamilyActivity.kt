...
class DoThiFamilyActivity : AppCompatActivity() {

    private lateinit var combinedChartFamily: CombinedChart
    private lateinit var dbRef: DatabaseReference
    private lateinit var auth: FirebaseAuth
    private lateinit var edtChonNamFamily: EditText
    private lateinit var btnTruyVanFamily: Button

    private lateinit var networkChangeReceiver: NetworkChangeReceiver

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_do_thi_family)

        combinedChartFamily = findViewById(R.id.combinedChartFamily)
        edtChonNamFamily = findViewById(R.id.edtChonNamFamily)
        btnTruyVanFamily = findViewById(R.id.btnTruyVanFamily)
        ...
        btnTruyVanFamily.setOnClickListener {
            val year = edtChonNamFamily.text.toString().trim()
            if (year.isNotEmpty()) {
                fetchChartData(year)
            } else {
                Toast.makeText(this@DoThiFamilyActivity, "Vui lòng chọn năm", Toast.LENGTH_SHORT).show()
            }
        }
        ...
        if (!initialNetworkStatus) {
            Toast.makeText(this@DoThiFamilyActivity, R.string.loss_internet, Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun fetchChartData(year: String) {
        ...
        currentUser?.let { user ->
            ...
            val thuRef = dbRef.child(userId!!).child("Family").child("Thu")
            val chiRef = dbRef.child(userId).child("Family").child("Chi")
