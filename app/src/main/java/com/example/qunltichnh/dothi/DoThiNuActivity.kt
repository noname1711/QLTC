...
class DoThiNuActivity : AppCompatActivity() {

    private lateinit var combinedChartNu: CombinedChart
    private lateinit var dbRef: DatabaseReference
    private lateinit var auth: FirebaseAuth
    private lateinit var edtChonNamNu: EditText
    private lateinit var btnTruyVanNu: Button

    private lateinit var networkChangeReceiver: NetworkChangeReceiver

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_do_thi_nu)

        combinedChartNu = findViewById(R.id.combinedChartNu)
        edtChonNamNu = findViewById(R.id.edtChonNamNu)
        btnTruyVanNu = findViewById(R.id.btnTruyVanNu)
        ...
        btnTruyVanNu.setOnClickListener {
            val year = edtChonNamNu.text.toString().trim()
            if (year.isNotEmpty()) {
                fetchChartData(year)
            } else {
                Toast.makeText(this@DoThiNuActivity, "Vui lòng chọn năm", Toast.LENGTH_SHORT).show()
            }
        }
        ...
        if (!initialNetworkStatus) {
            Toast.makeText(this@DoThiNuActivity, R.string.loss_internet, Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun fetchChartData(year: String) {
        ...
        currentUser?.let { user ->
            ...
            val thuRef = dbRef.child(userId!!).child("Nu").child("Thu")
            val chiRef = dbRef.child(userId).child("Nu").child("Chi")
