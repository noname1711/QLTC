...
class InsertionChiCompanyActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_insertion_chi_company)
        ...
        //nút thoát
        val imgBtnBackChi= findViewById<ImageButton>(R.id.imgBtnBackChiCompany)
        imgBtnBackChi.setOnClickListener{
            finish()
        }

        //pick thời gian mặc định là thực hoặc tùy chọn
        val btnLich = findViewById<ImageButton>(R.id.BtnLichChiCompany)
        val btnDongHo = findViewById<ImageButton>(R.id.BtnDongHoChiCompany)
        val txtTimeChi = findViewById<TextView>(R.id.txtTimeChiCompany)
        val txtDateChi = findViewById<TextView>(R.id.txtDateChiCompany)
        //thời gian thực
        ...
        // Nút chọn ảnh
        val btnAnhChi = findViewById<Button>(R.id.btnAnhChiCompany)
        btnAnhChi.setOnClickListener {
            showImagePickerDialog()
        }

        // Nút lưu thông tin
        val btnSaveChi = findViewById<Button>(R.id.btnSaveChiCompany)
    ...
    private fun uploadImage(uri: Uri) {
        ...
        currentUser?.let { user ->
            ...
            val imageRef = storageRef.child(userId).child("Company").child("Chi").child("image/${uri.lastPathSegment}")   //ảnh đi kèm với định dạng của ảnh
    ...
    private fun saveEmployeeData() {
        val currentUser = auth.currentUser
        val edtKhoanChi = findViewById<EditText>(R.id.edtKhoanChiCompany)
        val edtLuongChi = findViewById<EditText>(R.id.edtLuongChiCompany)
        currentUser?.let { user ->
            val userId = (user.email?: "").substringBefore("@")   //firebase ko nhận dấu @ và . nên chỉ lấy phần trước dấu @ của gmail
            val empDateChi = findViewById<TextView>(R.id.txtDateChiCompany).text.toString()
            val empTimeChi = findViewById<TextView>(R.id.txtTimeChiCompany).text.toString()
            val empKhoanChi = findViewById<EditText>(R.id.edtKhoanChiCompany).text.toString()
            val empLuongChi = findViewById<EditText>(R.id.edtLuongChiCompany).text.toString()
