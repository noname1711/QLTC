...
class InsertionChiFamilyActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_insertion_chi_family)
        ...
        //nút thoát
        val imgBtnBackChi= findViewById<ImageButton>(R.id.imgBtnBackFamilyChi)
        imgBtnBackChi.setOnClickListener{
            finish()
        }

        //pick thời gian mặc định là thực hoặc tùy chọn
        val btnLich = findViewById<ImageButton>(R.id.BtnLichFamilyChi)
        val btnDongHo = findViewById<ImageButton>(R.id.BtnDongHoFamilyChi)
        val txtTimeChi = findViewById<TextView>(R.id.txtTimeFamilyChi)
        val txtDateChi = findViewById<TextView>(R.id.txtDateFamilyChi)
        //thời gian thực
        ...
        // Nút chọn ảnh
        val btnAnhChi = findViewById<Button>(R.id.btnAnhChiFamily)
        btnAnhChi.setOnClickListener {
            showImagePickerDialog()
        }

        // Nút lưu thông tin
        val btnSaveChi = findViewById<Button>(R.id.btnSaveChiFamily)
    ...
    private fun uploadImage(uri: Uri) {
        ...
        currentUser?.let { user ->
            ...
            val imageRef = storageRef.child(userId).child("Family").child("Chi").child("image/${uri.lastPathSegment}")   //ảnh đi kèm với định dạng của ảnh
    ...
    private fun saveEmployeeData() {
        val currentUser = auth.currentUser
        val edtKhoanChi = findViewById<EditText>(R.id.edtKhoanChiFamily)
        val edtLuongChi = findViewById<EditText>(R.id.edtLuongChiFamily)
        currentUser?.let { user ->
            val userId = (user.email?: "").substringBefore("@")   //firebase ko nhận dấu @ và . nên chỉ lấy phần trước dấu @ của gmail
            val empDateChi = findViewById<TextView>(R.id.txtDateFamilyChi).text.toString()
            val empTimeChi = findViewById<TextView>(R.id.txtTimeFamilyChi).text.toString()
            val empKhoanChi = findViewById<EditText>(R.id.edtKhoanChiFamily).text.toString()
            val empLuongChi = findViewById<EditText>(R.id.edtLuongChiFamily).text.toString()
