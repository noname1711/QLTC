...
class InsertionThuCompanyActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_insertion_thu_company)
        ...
        //nút thoát
        val imgBtnBackCompany= findViewById<ImageButton>(R.id.imgBtnBackCompany)
        imgBtnBackCompany.setOnClickListener{
            finish()
        }
        //pick thời gian mặc định là thực hoặc tùy chọn
        val btnLich = findViewById<ImageButton>(R.id.BtnLichCompany)
        val btnDongHo = findViewById<ImageButton>(R.id.BtnDongHoCompany)
        val txtTime = findViewById<TextView>(R.id.txtTimeCompany)
        val txtDate = findViewById<TextView>(R.id.txtDateCompany)
        //thời gian thực
        ...
        // Nút chọn ảnh
        val btnAnhThu = findViewById<Button>(R.id.btnAnhThuCompany)
        btnAnhThu.setOnClickListener {
            showImagePickerDialog()
        }

        // Nút lưu thông tin
        val btnSaveThu = findViewById<Button>(R.id.btnSaveThuCompany)
    ...
    private fun uploadImage(uri: Uri) {
        ...
        currentUser?.let { user ->
            ...
            val imageRef = storageRef.child(userId).child("Company").child("Thu").child("image/${uri.lastPathSegment}")  //ảnh đi kèm với định dạng của ảnh
    ...
    private fun saveEmployeeData() {
        val currentUser = auth.currentUser
        val edtKhoanThu = findViewById<EditText>(R.id.edtKhoanThuCompany)
        val edtLuongThu = findViewById<EditText>(R.id.edtLuongThuCompany)

        currentUser?.let { user ->
            val userId = (user.email?: "").substringBefore("@")   //firebase ko nhận dấu @ và . nên chỉ lấy phần trước dấu @ của gmail
            val empDate = findViewById<TextView>(R.id.txtDateCompany).text.toString()
            val empTime = findViewById<TextView>(R.id.txtTimeCompany).text.toString()
            val empKhoanThu = findViewById<EditText>(R.id.edtKhoanThuCompany).text.toString()
            val empLuongThu = findViewById<EditText>(R.id.edtLuongThuCompany).text.toString()
